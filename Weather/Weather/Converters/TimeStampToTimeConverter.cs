    ...
    public class TimeStampToTimeConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value is ulong time)
            {
                ...
                {
                    date = TimeStamp.Convert(time).AddSeconds(timeZone.Offset).ToLocalTime()
                        .ToString(culture.DateTimeFormat.ShortTimePattern);
                }
                else
                {
                    date = TimeStamp.Convert((ulong)value).ToLocalTime().ToString(culture.DateTimeFormat.ShortTimePattern);
                }
            }
